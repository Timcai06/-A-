...
def build_report(result: TermStructureResult) -> str:
    if result.daily_metrics.empty:
        data_section = """当前未发现 `data/external/futures/布伦特期货多期限结算价_手工官方.csv`。因此本轮没有生成连续 M1-M3、M1-M6 数值输入，也没有把期限结构写入长期模型参数。"""
    else:
        latest = result.daily_metrics.dropna(subset=["front_month_price"]).tail(1).iloc[0]
        data_section = (
            f"已读取官方手工CSV并生成 {len(result.daily_metrics)} 个交易日的期限结构指标。"
...
- 有数据后，优先使用 M1-M3、M1-M6 价差和曲线斜率约束长期状态转移概率，不直接替代附件真实价格。
...
def main() -> None:
    ...
    if MANUAL_INPUT_CSV.exists():
        ...
        status = "waiting_for_official_multi_tenor_csv"
