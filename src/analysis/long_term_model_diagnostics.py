...
| 状态转移第180天P50 | {summary["p50_day180"]:.2f} | 概率路径中位数，反映制度风险残余 |
| 状态转移第180天P95 | {summary["p95_day180"]:.2f} | 后期上尾区间上沿 |
| 第90天P95-P05宽度 | {summary["width_day90"]:.2f} | 早中期不确定性仍较宽 |
| 第180天P95-P05宽度 | {summary["width_day180"]:.2f} | 后期概率扇形仍保留尾部 |
| 第90至180天宽度变化率 | {width_change:.1%} | 宽度收缩幅度 |
| 第90天升级状态占比 | {summary["escalation_day90"]:.1%} | 早中期仍有尾部冲击 |
| 第180天升级状态占比 | {summary["escalation_day180"]:.1%} | 后期升级状态得到保留 |
| 第180天“维持→升级”概率 | {summary["neutral_to_escalation_day180"]:.1%} | 后期仍保留再升级通道 |
...
2. **状态层已补足一部分尾部。** 第 180 天升级状态占比为 {summary["escalation_day180"]:.1%}，缓和状态占比为 {summary["easing_day180"]:.1%}，说明恢复证据门控已经避免了机械缓和。
3. **中性线不是训练出来的点预测。** 它来自供给恢复、SPR收缩、需求弹性恢复和风险溢价衰减的机制递推，因此平滑是模型结构结果，不代表它可以精确预测未来每一天。
4. **下一步应提升机制随机性。** GPR/OVX 将突破 120 概率提升 {summary["risk_p120_lift"]:.1%}，方向有效；后续更值得把随机供给恢复和库存耗尽二次跳涨接入中心路径，而不是继续堆防御性检验。
