...
本目录用于完成浙江工商大学 2026 年大学生数学建模竞赛 A 题。项目已经从“想法规划”进入“可复算建模”阶段：阶段 1 数据清洗和阶段 2 传统供需基准模型已经完成，下一步进入阶段 3 短期动态递推模型。
...
flowchart TB
    ...
    Dynamic["阶段3 下一步<br/>库存 / SPR / 绕道 / 需求收缩 / 恐慌衰减"]
    Paper["最终交付<br/>可复算模型 + 论文图表 + 结论证据链"]

    Goal --> Data --> Baseline --> Dynamic --> Paper
```
...
接下来阶段 3 不是重新找数据，而是在已有真实价格路径上解释机制：为什么供应少了那么多，价格却没有按传统模型涨到 200 美元以上。
