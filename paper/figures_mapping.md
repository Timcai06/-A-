...
当前 `figures/` 与 `paper/figures/` 中的核心图表已经进入 `paper/总论文.tex`，总论文是唯一主稿。

需要注意：图 1 和图 3 使用 2017-2026 全历史数据；图 2、图 4、图 5 聚焦 2026 冲突窗口，因此横轴只显示 2026 年附近的日期。

短期动态模型素材图存放在 `paper/figures/`。这些图已经择优迁移进总论文，用于支撑基准对比、滞后检验、拐点检验、机制消融、过拟合压力测试和稳健性带。
...
| 图8 | `figures/sensitivity_tornado_180day.png` | 阶段 6 | 敏感性分析 | 不确定性平台、地缘风险权重、供应中断量最敏感 | `output/sensitivity/阶段6_参数重要性排序.csv` | 已生成 |
