...
短期动态模型素材图存放在 `paper/figures/`。这些图已经择优迁移进总论文，用于支撑基准对比、滞后检验、拐点检验、机制消融、参数扰动检验和稳健性带；部分重复图保留为备用素材，不再全部塞入正文。

## 当前生成情况

| 状态 | 数量 | 图号 |
|---|---:|---|
| 阶段图 | 26 | 数据、基准、三情景、敏感性分析、蒙特卡洛、状态转移、GPR审计、OVX市场风险审计、历史稳健性、历史波动校准图、R学术增强图、在线残差校正图、时间轴与删块稳健性图、Mermaid结构图 |
...
| 结构图2 | `paper/figures/评审质疑防御框架图.png` | 全论文 | 基准对比与鲁棒性检验 | 展示随机游走、滞后复制、参数扰动、结构边界、内生性和长期可信度的证据链 | `paper/diagrams/评审质疑防御框架图.mmd` | 正式使用 |
...
| Mermaid检验图 | `paper/figures/评审质疑防御框架图.png` | 全论文 | 用结构化流程图展示模型鲁棒性与边界检验证据链 | `paper/diagrams/评审质疑防御框架图.mmd` | 已生成 |
