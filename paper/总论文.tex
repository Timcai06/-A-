...
换言之，长期部分的正确读法不是“第 180 天一定等于 98.61 美元/桶”，而是“在中性政策缓冲、供给恢复和风险溢价衰减假设下，中心路径收敛到约 100 美元/桶附近；若多个不利因素同时出现，价格仍可能进入 120 美元/桶以上的尾部区间”。因此，三情景中心线需要与第 \ref{sec:monte-carlo} 节蒙特卡洛扇形区间合读。
...
其中 $\Pi(\cdot)$ 表示由剩余供需缺口和长期需求弹性 $\varepsilon_L$ 共同决定的供需压力；当 $G_t^{(L)}<0$ 时，过剩供给通过较弱的折价项向下拉动目标价格。$\Phi_t$ 由地缘风险溢价、前期冲击不确定性和持续封锁制度风险三部分构成，$\Omega_t$ 为政策缓冲确认和预期修复带来的折价，$J_t$ 为库存接近耗尽时的跳变风险项。该形式对应赛题要求中的增产、长期弹性变化、运输瓶颈和库存耗尽风险。
...
蒙特卡洛情景树 & 第 180 天价格 P05--P95 为 91.67--108.31 美元/桶，最高价突破 120 概率约 8.0\% & 条件概率，不是无条件市场概率 \\
\bottomrule
\end{tabularx}
\end{table}
...
敏感性分析表明，第 180 天终点价格对市场价格形成层最敏感，尤其是不确定性与制度风险强度和地缘风险权重；供应中断量决定价格上行压力的物理底座。长期需求弹性在新的自适应外推中也具有较高影响，因为它决定高价持续后需求侧能否真正完成再平衡。SPR 释放、启动延迟和绕道运输在中性路径下对第 180 天终点价影响较小，但会改变外推期峰值、缺口闭合速度和二次跳涨幅度，因此更适合被解释为“削峰”和“争取恢复时间”的政策缓冲变量。
...
\begin{enumerate}
  ...
  \item 60--180 天情景预测显示，中性情景第 180 天价格约为 98.61 美元/桶；悲观情景第 180 天约为 114.49 美元/桶，并存在较高二次跳涨风险。
  \item 敏感性分析显示，不确定性与制度风险强度、地缘风险权重和供应中断量是最关键影响因素；SPR 和绕道运输更偏向削峰、缺口闭合和争取恢复时间。
  \item 蒙特卡洛情景树显示，第 180 天价格 5\%--95\% 区间为 91.67--108.31 美元/桶，外推期最高价突破 120 美元/桶的概率约为 8.0\%，尾部风险主要来自多因素同时恶化。
