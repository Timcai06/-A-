...
针对任务二，本文构建 60--180 天乐观、中性、悲观三种情景路径。新版长期自适应反馈模型显示，中性情景第 180 天价格约为 92.43 美元/桶，悲观情景第 180 天约为 106.83 美元/桶，且仍保留二次跳涨风险。敏感性分析表明，地缘风险权重、不确定性与制度风险强度、供应中断量是综合敏感度最高的三个因素；进一步的 2000 次蒙特卡洛情景树显示，外推期最高价突破 120 美元/桶的条件概率约为 4.6\%，突破 130 美元/桶的概率约为 0.0\%。此外，Caldara-Iacoviello GPR 指数显示 2026 年 3 月和 4 月地缘风险处于历史高分位，但滞后检验表明其不宜作为同月拟合输入。2017--2026 年同长度滚动窗口检验进一步显示，2026 冲突窗口累计收益率处于历史约 99.3\% 分位、实现波动率处于约 95.7\% 分位，说明该窗口对普通时间序列基准显著更难。本文据此将 GPR 和历史样本定位为外部审计证据，而非短期点预测因子。
...
为了避免把模型理解为单纯曲线拟合，本文将参数按来源分为五类。题面参数和附件统计量构成模型硬约束；官方外部数据只用于数量级审计；校准参数必须通过基准对比、消融实验和扰动检验；长期情景参数只用于 60--180 天条件外推，不参与短期窗口的误差拟合。
...
风险权重 $\beta$ & 2.462 & 不确定性/制度风险强度 & 0.247 \\
...
换言之，长期部分的正确读法不是“第 180 天一定等于 92.43 美元/桶”，而是“在中性政策缓冲、供给恢复和风险溢价衰减假设下，中心路径回归到低 90 美元/桶附近；若多个不利因素同时出现，价格仍可能进入 120 美元/桶以上的尾部区间”。因此，三情景中心线需要与第 \ref{sec:monte-carlo} 节蒙特卡洛扇形区间合读。
...
三情景参数并非三组孤立猜测。乐观情景代表高 SPR 可用量、绕道恢复较充分、长期需求调整较快；悲观情景代表供应中断接近上界、SPR 释放受限、绕道能力偏低且制度风险溢价持续；中性情景则采用阶段 4 综合最优参数附近的中心设定。后续敏感性分析和蒙特卡洛抽样用于检验：当这些假设联合变化时，价格中心路径和尾部风险是否仍保持一致。
...
结果显示，中性情景下价格在 180 天附近回落至约 92.43 美元/桶；悲观情景下，若供应中断偏大、SPR 释放弱、绕道恢复慢且风险溢价维持高位，价格仍可能再次升至 120 美元/桶以上，并在外推期达到约 128.64 美元/桶的高点。
...
3 & 供应中断量 & 5.95 & 3.26 & 5.97 \\
4 & 长期需求弹性 & 3.86 & 3.42 & 0.98 \\
5 & SPR 释放上限 & 0.90 & 0.65 & 0.50 \\
6 & 绕道运输能力 & 0.39 & 0.00 & 0.86 \\
...
敏感性分析表明，第 180 天终点价格对市场价格形成层最敏感，尤其是地缘风险权重和不确定性与制度风险强度；供应中断量决定价格上行压力的物理底座。长期需求弹性在新的自适应外推中也具有较高影响，因为它决定高价持续后需求侧能否真正完成再平衡。SPR 释放、启动延迟和绕道运输在中性路径下对第 180 天终点价影响相对较小，但会改变外推期峰值、缺口闭合速度和二次跳涨幅度，因此更适合被解释为“削峰”和“争取恢复时间”的政策缓冲变量。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.94\linewidth]{monte_carlo_price_fan.png}
\caption{蒙特卡洛情景树价格扇形区间。深色区间为 25\%--75\%，浅色区间为 5\%--95\%。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{monte_carlo_tail_risk.png}
\caption{蒙特卡洛尾部风险概率。}
\end{figure}

结果表明，蒙特卡洛路径的第 180 天中位数价格为 92.85 美元/桶，与阶段 5 中性情景的 92.43 美元/桶高度接近，说明中性路径不是孤立的单点调参结果。与此同时，外推期最高价突破 120 美元/桶的概率仍有 4.6\%，且高压尾部路径中突破 120 美元/桶的概率达到 21.1\%。这说明长期风险的重点不是中性点预测本身，而是多因素联动恶化下的尾部组合风险。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{gpr_event_window_audit.png}
\caption{地缘风险指数审计：同步 GPR 与滞后可用 GPR。3 月油价解释不能使用 3 月同步 GPR，4 月以后才可使用已实现的 3 月风险读数。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.84\linewidth]{lagged_gpr_brent_return.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{historical_event_extremeness.png}
\caption{2026 冲突窗口在历史同长度窗口中的极端性。累计涨幅、波动率和朴素基准误差均处于历史高分位。}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\linewidth]{historical_baseline_error_distribution.png}
\caption{历史 46 交易日窗口的随机游走基准误差分布。冲突窗口朴素基准 RMSE 处于历史高位，而本文机制模型在该窗口内仍明显优于朴素基准。}
\end{figure}
...
\begin{enumerate}
  ...
  \item 主动回应评委质疑：本文设置 Random Walk 基准、ARIMA 基准、滞后平移检验、机制消融、过拟合压力测试和历史同长度窗口检验。
  \item 数据口径保守：所有真实价格均来自附件 CSV；外推阶段明确标注为情景预测，不作为真实观测。
  \item 长期表述审慎：三情景中心线与蒙特卡洛扇形区间合读，把长期结果定位为条件中心路径和尾部风险，而非精确日度点预测。
\end{enumerate}
...
\begin{enumerate}
  ...
  \item 蒙特卡洛情景树已经刻画了部分参数联动风险，但其联合分布仍来自模型设定而非外部历史频率估计，因此尾部概率应解释为条件概率而非真实市场概率。
\end{enumerate}

\subsection{改进方向}

若后续继续深化，可以从三个方向提高模型外推能力：第一，在现有 EIA 周度数据审计基础上，继续接入 IEA 或 OPEC 全球口径供需平衡表，对长期供给恢复路径进行更客观的外生约束；第二，在现有 GPR 滞后审计基础上，继续探索期权隐含波动率、油轮保险费、期货期限结构等更接近市场定价的风险变量；第三，将当前蒙特卡洛联合扰动进一步扩展为带状态转移概率的情景树，使冲突缓和、维持和升级三类状态可以随时间动态切换。R 语言可用于后续 GARCH、VAR、Granger 和稳健标准误等计量审计，C++ 多智能体仿真则只有在获得主体级行为数据时才有必要进入主线。
...
\begin{enumerate}
  ...
  \item 敏感性分析显示，地缘风险权重、不确定性与制度风险强度和供应中断量是最关键影响因素；SPR 和绕道运输更偏向削峰、缺口闭合和争取恢复时间。
  \item 蒙特卡洛情景树显示，第 180 天价格 5\%--95\% 区间为 87.67--101.01 美元/桶，外推期最高价突破 120 美元/桶的条件概率约为 4.6\%，尾部风险主要来自多因素同时恶化。
