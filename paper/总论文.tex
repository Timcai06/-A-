...
霍尔木兹海峡是全球原油运输的关键咽喉。一旦发生封锁，供应中断、地缘风险重估、战略储备释放、商业库存缓冲、绕道运输恢复和市场预期修复会同时作用于国际原油价格。本文以附件给出的布伦特原油期货主力合约价格数据为基础，建立了“附件数据清洗--传统供需基准--短期动态递推--长期情景预测--敏感性分析”的完整模型链条。

针对任务一，本文首先清洗 2017 年 9 月至 2026 年 5 月的原始价格序列，并截取 2026 年 3 月 2 日至 2026 年 5 月 5 日的冲突窗口作为主要拟合区间。附件数据显示，冲突窗口内最高收盘价为 114.06 美元/桶，最高盘中价为 119.50 美元/桶。随后，本文按题面给定的短期低需求弹性和供应中断范围建立传统供需基准模型，得到约 278--337 美元/桶的理论价格区间。该结果显著高于附件真实价格，说明单纯低弹性供需框架会高估现实价格，必须进一步刻画政策缓冲和市场预期修复。

针对任务一中的日度价格解释，本文构建短期动态递推模型。模型在物理供需层纳入供应中断、SPR 释放、商业库存、绕道运输、需求收缩和恐慌衰减，在市场价格形成层纳入地缘风险溢价、不确定性平台、缓冲确认折价和预期修复折价。通过固定种子随机搜索、差分进化连续精修、局部稳健性复核和拟合质量精修，综合最优模型在 46 个交易日冲突窗口内取得 RMSE 为 3.44 美元/桶、MAE 为 2.85 美元/桶、MAPE 为 2.86\% 的拟合效果，并优于上一日价格朴素基准和滚动 ARIMA 基准。滞后平移检验、拐点检验、机制消融、过拟合压力测试和硬编码审计表明，该模型不是简单滞后复制，也没有把 120 美元平台写成代码上限。

针对任务二，本文在短期机制校准结果基础上构建 60--180 天乐观、中性、悲观三种情景路径。中性情景第 180 天价格约为 104.49 美元/桶，悲观情景第 180 天约为 119.53 美元/桶，且存在较高二次跳涨风险。最后，本文围绕 10 个关键参数进行单因素敏感性分析，发现综合敏感度最高的三个因素为不确定性平台、地缘风险权重和供应中断量；SPR 释放和绕道运输对第 180 天终点价格影响较小，但能够影响外推期峰值和削峰能力。研究结果表明，短期油价平台由物理缺口和市场风险重估共同决定，长期风险则取决于高风险溢价、供应中断规模和政策缓冲能否形成组合对冲。

\textbf{关键词：} 霍尔木兹海峡；原油价格；动态递推模型；战略石油储备；情景预测；敏感性分析

\begin{center}
\fcolorbox{OilBlue}{OilLight}{%
\begin{minipage}{0.92\linewidth}
\textbf{核心结论：}
传统低弹性供需模型会显著高估封锁后的油价上界；加入 SPR、库存、绕道、需求收缩和市场预期修复后，动态模型可以解释现实价格为何停留在约 110 美元/桶平台。若封锁延长至 180 天，中性情景价格约为 104.49 美元/桶，悲观情景约为 119.53 美元/桶；最关键的不确定因素是市场不确定性平台、地缘风险权重和供应中断量。
\end{minipage}}
\end{center}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{短期模型误差诊断.png}
\caption{短期模型逐日误差与分段误差诊断。误差没有集中失控于单一阶段。}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{scenario_price_paths.png}
\caption{阶段 5：60--180 天三情景原油价格路径。}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{sensitivity_parameter_response.png}
\caption{前三个高敏感参数响应曲线。}
\end{figure}
