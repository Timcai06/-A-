...
\subsection*{C 短期冲击模型补充检验}

正文第七章保留校准、基准对比和鲁棒性摘要。附录补充给出短期模型的机制贡献、参数剖面、事件窗口、滞后平移、残差分布和局部扰动稳健性图。
...
\subsection*{D 敏感性与外生风险补充检验}

正文第九章只保留参数排序、尾部风险和外生风险摘要。附录在短期模型检验之后补充长期敏感性和外生风险证据，主要用于说明扰动尺度从何而来、外部风险变量如何进入解释，以及 2026 年冲突窗口相对历史窗口是否具有特殊性。
...
\appendixfigunit{0.41\textheight}{传统蒙特卡洛路径云}{该图展示不引入状态转移结构时，多参数联合扰动形成的价格路径云。它作为长期情景树的朴素随机模拟参照，用来区分普通参数扰动和状态切换风险。}{0.88}{0.24\textheight}{传统蒙特卡洛路径云图.png}{附图 8 传统蒙特卡洛路径云图}
...
\subsection*{E 短期冲击模型核心代码}

代码~\ref{lst:dynamic-recursion} 展示短期冲击模型的核心实现。
...
def simulate_dynamic_model(event_df, assumptions, behavior):
    ...
    for step_index, actual_row in enumerate(event_df.itertuples(index=False)):
        ...
        # 需求侧：价格弹性先解释一部分需求下降，题面需求收缩只扣除净额
        ...
        elasticity_loss = max(assumptions.base_demand - price_adjusted_demand, 0.0)
        demand_decline = max(observed_decline - 0.35 * elasticity_loss, 0.0)
...
短期拟合误差 & RMSE = 3.37 美元/桶，MAE = 2.74 美元/桶，MAPE = 2.73\% \\
峰值误差 & 模拟峰值 110.49 美元/桶，实际最高收盘价 114.06 美元/桶 \\
基准对比 & 主模型优于朴素上一日基准和滚动 ARIMA 基准 \\
长期中性结果 & 第 180 天价格约 94.47 美元/桶 \\
长期悲观结果 & 第 180 天价格约 115.90 美元/桶，外推期保留二次跳涨风险 \\
尾部风险 & 状态转移情景树显示外推期最高价突破 120 美元/桶的条件概率约 76.3\% \\
