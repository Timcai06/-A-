...
代码~\ref{lst:state-transition-probability} 展示长期预测中状态转移概率的递推方式。基础矩阵先给出缓和、维持、升级三类状态之间的常规切换概率，再由物理压力和 GPR/OVX 综合风险压力对升级概率进行温和修正；后期缓和不再按日历时间机械触发，而由恢复证据门控。
...
长期中性结果 & 第 180 天价格约 93.26 美元/桶 \\
长期悲观结果 & 第 180 天价格约 111.71 美元/桶，外推期保留二次跳涨风险 \\
尾部风险 & 状态转移情景树显示外推期最高价突破 120 美元/桶的条件概率约 44.0\% \\
