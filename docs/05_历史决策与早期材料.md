...
- 阶段 1 已新增并运行
  `src/pipeline/validate_stage1.py --rerun`，自动验收 33 项，全部通过。
...
- 阶段 5 已实现并运行 `src/scenarios/forecast_stage5.py`，基于阶段 4
  综合最优参数构造乐观、中性、悲观三条 60-180 天路径。
- 阶段 5 最新结论：阶段 12 后采用新版长期自适应反馈主模型，中性情景第 180 天约 92.43 USD/barrel；悲观情景第 180 天约
  106.83 USD/barrel，且存在二次跳涨风险。早期 98.61/114.49 口径已降级为历史审计材料。
- 阶段 6 已实现并运行 `src/analysis/sensitivity_stage6.py`，围绕 10
  个关键参数做单因素敏感性分析。
- 阶段 6
  结论：综合敏感度前三为不确定性平台、地缘风险权重、供应中断量；SPR
...
- 阶段 8 已完成阶段 5
  长期情景脚本拆分：`src/scenarios/forecast_stage5.py`
  保留原命令入口和兼容导出，具体职责拆到
...
| [../output/final/总论文.pdf](../output/final/总论文.pdf) | 已编译并渲染抽查的总论文 PDF 初稿；当前正文预览为 26 页，后续附录会增加页数 |
...
| `output/final/总论文.pdf` | 已编译并渲染抽查的总论文 PDF 初稿；当前正文预览为 26 页，后续加入代码附录后会增加 |
...
| `src/analysis/factor_selection_stage8.py` | 生成因素覆盖矩阵 CSV 与 Markdown 报告 |
...
当前 PDF 为 `output/final/总论文.pdf`，正文预览共 26 页；当前 DOCX 为
...
1.  拆 `src/scenarios/forecast_stage5.py`：
