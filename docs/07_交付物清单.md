...
| `output/pdf/短期动态模型论文.pdf` | 已编译并渲染检查的 13 页短期动态模型 PDF 初稿 |
| `output/doc/短期动态模型论文.docx` | 已导出的短期动态模型 Word 版本，用于后续修改、批注和提交 DOCX |
...
当前 PDF 为 `output/pdf/短期动态模型论文.pdf`，共 13 页；当前 DOCX 为 `output/doc/短期动态模型论文.docx`。PDF 作为正式排版版本，DOCX 作为可编辑版本，二者都可作为最终总论文中“短期动态模型、参数校准与结果解释”部分的底稿。
