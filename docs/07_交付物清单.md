...
当前 PDF 为 `output/final/短期动态模型论文.pdf`；当前 DOCX 为 `output/final/短期动态模型论文.docx`。PDF 作为正式排版版本，DOCX 作为可编辑版本，二者都可作为最终总论文中“短期动态模型、参数校准与结果解释”部分的底稿。
...
当前 PDF 为 `output/final/总论文.pdf`，共 18 页；当前 DOCX 为 `output/final/总论文.docx`。PDF 适合作为当前正式排版预览，DOCX 适合给队友批注和人工改写。
