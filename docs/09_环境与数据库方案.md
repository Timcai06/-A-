...
本文档记录阶段 0.6 的环境与 PostgreSQL 决策。当前只完成设计和文件准备，不立即建库、不写入数据。
...
当前阶段还没有进入数据清洗。直接建库会带来两个问题：

1. 表结构可能随阶段 1 的真实清洗结果调整。
2. 过早写库会让简单问题复杂化，拖慢模型主线。
...
- `data/processed/brent_daily_clean.csv` 已生成。
- `data/processed/brent_event_window.csv` 已生成。
- 字段名、缺失值处理和日期范围已经稳定。
- 阶段 1 验收报告已经完成。
